...
void store_messages_to_file( std::vector<vtr_vision_msgs::msg::ChannelLandmarks> ChannelLandmarks, const std::string& output_dir, int max_messages = 5) {
    ...
    for (int i = 0; i < max_messages; ++i) {
        ...
        std::vector<uint8_t> desc = ChannelLandmarks[i].descriptors;
        std::vector<bool> valid = ChannelLandmarks[i].valid;
        
        // Create filename with index
        std::string filename = output_dir + "/desc_" + std::to_string(i).insert(0, 5 - std::to_string(i).length(), '0') + ".txt";
...
int main(int argc, char** argv) {
    ...
    std::string output_directory = "/home/adam/Desktop/CurrentBranch/src/main/src/vtr_db_extractor/descriptor_results";
