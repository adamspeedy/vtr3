...
Navigator::Navigator(const rclcpp::Node::SharedPtr& node) : node_(node) {
  ...
  /// pose graph
  auto new_graph = node_->declare_parameter<bool>("start_new_graph", true);
  node_->get_parameter("start_new_graph", new_graph);
  CLOG(INFO, "navigation") << "start new graph set to: " << new_graph;
  ...
  /// path planner
  CLOG(INFO, "navigation") << "Setting up Path Planner";
...
if (pipeline->name() == "stereo") {
  ...
  if (use_odom_topic) {
    ...
    zed_odom_sub_.subscribe(node_, odom_topic, camera_qos.get_rmw_qos_profile());
