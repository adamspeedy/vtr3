...
void StereoPipeline::runOdometry_(const tactic::QueryCache::Ptr &qdata0, const tactic::OutputCache::Ptr &output0,
  ...
  // CLOG(INFO, "stereo.pipeline") << "T_r_v_odo set" << *qdata.T_r_v_odo;
  // CLOG(INFO, "stereo.pipeline") << "First frame flag: " << *qdata.first_frame ; // this is set at the start of the run
  ...
  if (use_odom_topic)
  {
    ...
    CLOG(DEBUG, "stereo.odom_debug") << "zed T_r_v is " << temp2;
  }
  else
  {
    qdata.T_r_v_odo = *qdata.T_r_m;
    // CLOG(DEBUG, "stereo.odom_debug") << "original T_r_v is " << qdata.T_r_v_odo[0];
  }
